(function($) {
    $.fn.fancyzoom = function(userOptions) {
        ...
        function openZoomBox(imgSrc, o) {
            if (o.showoverlay) { oOverlay.appendTo('body').click(function() { closeZoomBox(o) }); if ($.browser.msie && $.browser.version < 7) { oOverlay.css({ position: 'absolute', height: $(document).height(), width: $(document).width() }) } }
            ...
            if (!imgSrc.is('img')) { o.dimOri = $.extend(o.dimOri, { width: 0, height: 0 }) }
            ...
            if (o.Speed > 0) { oImgZoomBox.css(o.dimOri).animate(dimBoxTarget, o.Speed, $fctEnd) } else {
        ...
        function closeZoomBox(o) {
            ...
            if (o.Speed > 0) {
                ...
                oImgZoomBox.animate(oDimPlus, o.Speed * 0.2, function() { oImgZoomBox.animate(o.dimOri, o.Speed, function() { endClose() }); if (o.showoverlay) { oOverlay.animate({ 'opacity': 0 }, o.Speed, function() { $(this).remove() }) } })
            } else { endClose(); if (o.showoverlay) { oOverlay.remove() } }
        }
        return this.each(function() {
            ...
            if (!/\.jpg|\.jpeg|\.png|\.gif/i.test(imgTargetSrc)) { return true }
            $this.click(function() {
                var zoomOpened = $('div.jqfancyzoombox');
                if (zoomOpened.length > 0) { if ($('img:first', zoomOpened).attr('src') != imgTargetSrc) { if (oLoading && oLoading.is(':visible')) { __cancelLoading() } } else { return false } }
                ...
                var $fctEndLoading = function() {
                    if (bCancelLoading) { bCancelLoading = false } else {
                ...
                var fctCalculateImageSize = function(autoresize) {
                    if (autoresize) {
                        ...
                        if (bResize && o.imgResizeScript) {
                            ...
                            for (i = 0; i < tabSizes.length; i++) { if (tWidth > tabSizes[i]) { finalWidth = tabSizes[i]; break } }
    ...
    $.fn.fancyzoom.defaultsOptions = { overlayColor: '#000', overlay: 0.6, imagezindex: 100, showoverlay: true, Speed: 400, shadow: true, shadowOpts: { color: "#000", offset: 4, opacity: 0.2 }, imgDir: 'Images/', imgResizeScript: null, autoresize: true };
    ...
    function __displayLoading(o) {
        if (!oLoading) { oLoading = $('<div></div>').css({ width: 50, height: 50, position: 'absolute', 'background': 'transparent', opacity: 8 / 10, color: '#FFF', padding: '5px', 'font-size': '10px' }) }
    ...
    function __changeimageLoading(o) {
        if (oLoading && !oLoading.is(':visible')) { timerLoadingImg = null; return }
        var $im = $('img', oLoading);
        if (!$im.attr('src') || /blank\.gif/.test($im.attr('src'))) { strImgSrc = o.imgDir + "zoom-spin-1.png" } else {
        ...
        if (pLoad.complete) { $fct() } else { pLoad.onload = $fct }
    }

    function __getFileName(strPath) { if (!strPath) { return false } var tabPath = strPath.split('/'); return ((tabPath.length < 1) ? strPath : tabPath[(tabPath.length - 1)]) }
})(jQuery);