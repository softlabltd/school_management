(function($, window, document, undefined) {
    var helpers = {
        ...
        init: function(element, options) {
            var target = null;
            if (typeof moment === "undefined") { target = new Date(options.target) } else {
            ...
            if (typeof $("body").data("countdownCubeCounter") == "undefined") { $("body").data("countdownCubeCounter", 0) } else { $("body").data("countdownCubeCounter", $("body").data("countdownCubeCounter") + 1) }
            ...
            for (var tagIndex in this.topTags) {
                ...
                cube = element.append("<section></section>").children(":last").attr("id", tag).addClass("countdownCubeContainer").append("<div></div>").children(":last").addClass("countdownCubeCube").data("side", "show-front");
                this.addFigures(cube, this.classes, this.loadingTags[tagIndex]);
                element.children(":last").append("<div></div>").children(":last").html(options.labelsTranslations[tag]).addClass("countdownCubeTitleDiv")
            }
            this.setupAllCss(element, options);
            var that = this;
            setTimeout(function() { that.setTimeLeft(element, options) }, 10);
            var refreshId = setInterval(function() { that.setTimeLeft(element, options) }, 1e3);
            element.data("countdownCubeId", refreshId);
            if (!this.has3d()) { element.append("<p>Your Browser/Computer Sucks - It doesnt support 3d Transforms!</p>") }
        },
        setupAllCss: function(element, options) {
            element.css({ diaplay: "inline-block" });
            ...
            $("." + this.elementClass + " .countdownCubeContainer").css({ height: cubeSizepx, width: cubeSizepx, "-webkit-perspective": perspective, "-moz-perspective": perspective, "-o-perspective": perspective, perspective: perspective });
            element.find(".countdownCubeTitleDiv").css({ "margin-top": cubeSizepx, "padding-top": "10px", "text-align": "center", "font-size": titleFontSize });
            element.find("figure").css({ margin: 0, height: figurepx, width: figurepx, "line-height": figurepx, "font-size": fontSize, background: options.background, color: options.color });
        ...
        createStyleSheet: function(element, childSelector, transform) {
            ...
            for (var index in this.transformNames) newCssProperties += this.transformNames[index] + ": " + transform + ";";
            newCssProperties += "}";
        ...
        addFigures: function(cube, classes, loadingTag) { var i; for (i = 0; i < 6; i++) { cube.append("<figure></figure>").children(":last").addClass(classes[i]).html(loadingTag) } return this },
        ...
        setTimeLeft: function(element, options) {
            ...
            if (!options.showDaysOnly) { var monthCube = element.find("#month .countdownCubeCube"); var yearCube = element.find("#year .countdownCubeCube") }
            var now = new Date;
            var diff = (target - now) / 1e3;
            if (diff < 0) {
                if (typeof element.data("countdownCube") != "undefined") {
                    ...
                    if (!element.timeEnded) {
                        ...
                        if (diff > -1e3) { if (!element.onEndCallbackTriggered) { this.onEndCallback(element, options) } } else { if (options.triggerEnd && !element.onEndCallbackTriggered) { this.onEndCallback(element, options) } }
            ...
            if (options.showDaysOnly) {
                yearsToShow = 0;
                monthsToShow = 0;
                days = Math.floor((target - now) / (1e3 * 3600 * 24));
                if (days == 0) { daysToShow = 0 } else { daysToShow = days }
            } else {
                if (days < 0 || days == 0 && hours < 0 || days == 0 && hours == 0 && minutes < 0 || days == 0 && hours == 0 && minutes == 0 && seconds <= 0) {
                    ...
                    var diffToNextMonth = Math.round((copy - now) / 1e3 / 3600 / 24) - 1;
                    daysToShow = target.getDate() + diffToNextMonth;
                    monthsToShow--
                } else if (days == 0) { daysToShow = 0 } else { daysToShow = days }
                if (hours < 0 || hours == 0 && minutes < 0 || hours == 0 && minutes == 0 && seconds <= 0) { daysToShow-- }
            }
            hoursToShow = hours;
            minutesToShow = minutes;
            secondsToShow = seconds;
            if (minutes < 0 || minutes == 0 && seconds <= 0) { hoursToShow-- }
            if (seconds <= 0) { minutesToShow-- }
            hoursToShow = (hoursToShow + 24) % 24;
            minutesToShow = (minutesToShow + 60) % 60;
            secondsToShow = secondsToShow + 60;
            if (years == 0 && months == 0 && days == 0 && hours == 0 && (minutes == 1 && seconds == 0 || minutes == 0 && seconds == 60)) {
                hoursToShow = 0;
                minutesToShow = 0;
                secondsToShow = 60
            } else {
                secondsToShow = secondsToShow % 60;
                diffTime = new Date(yearsToShow, monthsToShow, daysToShow, hoursToShow, minutesToShow, secondsToShow, 0);
        ...
        onEndCallback: function(element, options) {
            ...
            $.event.trigger({ type: "countertimeEnded", source: element.context.id, options: options, time: new Date });
    ...
    $.fn["countdownCube"] = function(options) { return this.each(function() { Plugin($(this), options) }) };
    $.fn.countdownCube.defaults = { targetTimezone: "UTC", cubeSize: 50, background: "rgba( 255, 150, 150, 0.8 )", color: "white", labelsTranslations: { year: "years", month: "months", day: "days", hour: "hours", minute: "minutes", second: "seconds" }, showDaysOnly: false, onEnd: function(e) { return }, triggerEnd: false }
